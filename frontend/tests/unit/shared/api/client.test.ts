...
describe("ApiClient", () => {
  ...
  describe("认证", () => {
    ...
    it("应从 authStore 内存自动获取 token", async () => {
      // 先重置模块，再设置 mock，再导入 client（确保 accessor 预加载使用 mock）
      vi.resetModules();
      vi.doMock("@features/auth/store/authStore", () => ({
        useAuthStore: {
          getState: () => ({ accessToken: "memory-token" }),
        },
      }));

      const mod = await import("@shared/api/client");
      const freshClient = new mod.ApiClient("https://api.test.com");

      // 等待 accessor 预加载 microtask 完成
      await new Promise((r) => setTimeout(r, 0));

      mockFetch.mockResolvedValue(createResponse({ items: [] }));
      await freshClient.get("/training-jobs");

      const [, options] = mockFetch.mock.calls[0];
      expect(options.headers["Authorization"]).toBe("Bearer memory-token");
    });

    it("显式设置的 token 应优先于 authStore", async () => {
      // 先重置模块，再设置 mock，再导入 client
      vi.resetModules();
      vi.doMock("@features/auth/store/authStore", () => ({
        useAuthStore: {
          getState: () => ({ accessToken: "memory-token" }),
      ...
      freshClient.setAuthToken("explicit-token");
      await freshClient.get("/training-jobs");
  ...
  describe("upload", () => {
    ...
    it("应支持自定义字段名和附加数据", async () => {
      const file = new File(["content"], "model.pt", {
        type: "application/octet-stream",
      });
      mockFetch.mockResolvedValue({
        ok: true,
