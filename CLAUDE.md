...
- **产品架构 (WHAT/WHY)**: 平台定位、五层业务架构、LLMOps 全生命周期、多租户模型 → 见 `.claude/rules/product-architecture.md`
- **技术架构 (HOW)**: 各子项目的代码分层与依赖方向（下表）
...
> 产品架构（业务视角）参见 `.claude/rules/product-architecture.md`
> 各子项目详细技术架构规范参见对应的 `.claude/rules/architecture.md`
...
| 文档 | 位置 | 用途 |
|------|------|------|
| **产品架构规范** | `.claude/rules/product-architecture.md` | 平台定位、五层业务架构、LLMOps 生命周期、多租户模型 |
...
> 产品架构: `.claude/rules/product-architecture.md` (业务视角的平台架构 SSOT)
