...
### Task 0.1: 配额删除能力决策 ✅ 已确认

**已定（2026-06-13 用户确认）：纳入本轮。** 核实后发现后端 `DELETE /resource-limit-configs/{id}` 已实现，故落地为纯前端任务，见 **Task 3B.0**。本任务无需再执行，保留记录。
...
### Task 3B.0: 配额删除能力（前端补全 + 后端契约确认）

> **决策已定（用户确认纳入本轮）**。核实结论：**后端删除能力已完整存在**——`DELETE /api/v1/resource-limit-configs/{id}`（204，`require_admin`，软删除）+ `service.delete_config()` 均已实现。因此删除是**纯前端任务** + 一条后端契约确认测试。删除为危险操作，需二次确认 Modal（参照 `spaces/pages/SpaceListPage.tsx` 的删除确认范式）。

**Files:**
- Modify: `frontend/src/features/resource-quotas/api/resourceQuotasApi.ts`（加 `deleteResourceLimitConfig`）
- Modify: `frontend/src/features/resource-quotas/api/queries.ts`（加 `useDeleteResourceLimitConfig`）
- Modify: `frontend/src/features/resource-quotas/ResourceQuotasPage.tsx`（表格操作列加"删除"按钮 + 确认 Modal）
- Test（后端契约）: `backend/tests/integration/quotas/test_api_resource_limit_config.py`（确认 DELETE 204 + 404，若已有则跳过）
- Test（前端单元）: `frontend/tests/unit/features/resource-quotas/`（hook + 页面删除交互）

- [ ] **Step 1: 后端契约确认测试**（先确认 DELETE 端点行为，可能已有测试）

Run: `cd backend && pytest tests/integration/quotas/ -v -k delete`
若无删除测试则补：admin 删除返回 204、删除不存在 ID 返回 404、非 admin 返回 403。Expected: PASS（后端已实现）。

- [ ] **Step 2: 写前端 api 失败测试 + 实现 `deleteResourceLimitConfig`**

```typescript
// resourceQuotasApi.ts
export async function deleteResourceLimitConfig(id: number): Promise<void> {
  return apiClient.delete(`/resource-limit-configs/${id}`);
}
```

- [ ] **Step 3: 写 hook 失败测试 + 实现 `useDeleteResourceLimitConfig`**（成功后 invalidate 列表）
...
export function useDeleteResourceLimitConfig() {
  ...
  return useMutation({
    mutationFn: deleteResourceLimitConfig,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.resourceQuotas.lists() });
    },
  });
}
```

- [ ] **Step 4: 页面删除按钮 + 确认 Modal**

操作列在"编辑"旁加"删除"按钮（`variant="inline-link"`）；点击弹 Cloudscape `Modal` 二次确认（`Alert type="warning"` + "确认删除"主按钮），确认后调 `useDeleteResourceLimitConfig`，成功显示 Flashbar 并刷新列表。写组件渲染 + 交互单元测试（点击删除→Modal 出现→确认→mutation 调用）。

- [ ] **Step 5: 运行前端单元测试 + lint**

Run: `cd frontend && npm test -- tests/unit/features/resource-quotas/ && npm run lint`
Expected: PASS

- [ ] **Step 6: 提交**

```bash
git add frontend/src/features/resource-quotas/ frontend/tests/unit/features/resource-quotas/ backend/tests/integration/quotas/
git commit -m "feat(frontend): 配额管理页删除能力（按钮 + 二次确认 Modal）"
```
...
- [ ] **Step 5: 追加删除 E2E**（依赖 Task 3B.0 前端删除能力）—— 创建一条测试配额 → 删除 → 确认 Modal → 验证列表中消失 + 计数 -1。真实环境用 admin 账号（删除需 admin 权限）。
...
- [ ] 配额页：真实环境 Playwright 套件全绿，含分页/校验边界/计数自洽/错误态/删除（含二次确认 Modal）
...
| 决策 | 结论 | 时间 |
|------|------|------|
| 配额删除能力是否纳入 | **纳入**（Task 3B.0）。后端已实现 DELETE，仅需补前端按钮+确认 Modal+E2E | 2026-06-13 用户确认 |
