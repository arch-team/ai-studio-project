...
**测试命令**：
- 后端单元：`cd backend && pytest tests/unit/modules/monitoring -v`
...
**Files:**
- Modify: `infrastructure/cdk/stacks/foundation/iam_stack.py`（`_create_backend_service_role`，在 `CloudWatchLogsAccess` policy 之后、`return role` 之前插入，约 line 357-369 区域）
- Test: `infrastructure/cdk/tests/unit/stacks/foundation/test_iam_stack.py`（若不存在则在对应测试文件追加）

- [ ] **Step 1: 写失败测试** —— 断言 backend role 含 aps 权限

```python
# 在 iam_stack 测试中追加（参考现有 backend role 测试断言风格）
def test_backend_role_has_amp_query_permissions(backend_iam_template):
    """后端服务角色应具备 AMP 查询权限以接通监控页指标。"""
    backend_iam_template.has_resource_properties(
        "AWS::IAM::Policy",
        {
            "PolicyDocument": {
...
Run: `cd infrastructure/cdk && pytest tests/unit/stacks/foundation/test_iam_stack.py::test_backend_role_has_amp_query_permissions -v`
Expected: FAIL（当前角色无 aps 权限）

- [ ] **Step 3: 实现 —— 插入 aps policy statement**
...
- Test: `backend/tests/unit/modules/monitoring/test_prometheus_sigv4.py`（新建）
...
Run: `cd backend && pytest tests/unit/modules/monitoring/test_prometheus_sigv4.py -v`
...
class PrometheusClient(IPrometheusClient):
    def __init__(self, endpoint: str | None = None, timeout: float = 30.0, use_sigv4: bool | None = None):
        ...
        # AMP 端点优先；否则回退本地 prometheus
        self._endpoint = (endpoint or settings.amp_query_endpoint or "http://localhost:9090").rstrip("/")
...
- [ ] **Step 5: 回归原有 prometheus 测试**

Run: `cd backend && pytest tests/unit/monitoring/test_svc_prometheus.py -v`
Expected: PASS（本地端点分支不签名，行为不变）

- [ ] **Step 6: 提交**

```bash
git add backend/src/modules/monitoring/infrastructure/external/prometheus_client.py backend/tests/unit/modules/monitoring/test_prometheus_sigv4.py
...
- Test: `backend/tests/unit/modules/monitoring/test_schema_monitoring.py`（新建）
...
Run: `cd backend && pytest tests/unit/modules/monitoring/test_schema_monitoring.py -v`
...
Run: `cd backend && pytest tests/unit/modules/monitoring/test_schema_monitoring.py -v && mypy src/modules/monitoring/api/schemas/`
Expected: PASS

- [ ] **Step 5: 提交**

```bash
git add backend/src/modules/monitoring/api/schemas/responses.py backend/tests/unit/modules/monitoring/test_schema_monitoring.py
git commit -m "feat(backend): 监控页响应 schema（集群列表/利用率/指标序列/告警）"
```

### Task 2C.2: 集群读穿透服务（DB 缓存 + 回源 SageMaker）
...
- Test: `backend/tests/unit/modules/monitoring/test_svc_cluster_sync.py`（新建）

- [ ] **Step 1: 写失败测试** —— 缺失/过期时回源、命中时不回源、并发单飞
...
async def test_get_clusters_refetches_when_db_empty():
    ...
    mock_repo = AsyncMock()
    mock_repo.list_all.return_value = []  # DB 空
    ...
    mock_repo.upsert.assert_awaited()                  # 写库
    assert len(result) >= 1
...
async def test_get_clusters_uses_db_when_fresh():
    """DB 有新鲜记录时不回源。"""
    mock_repo = AsyncMock()
    mock_repo.list_all.return_value = [_fresh_cluster()]  # last_sync_at 在 TTL 内
    mock_sagemaker = AsyncMock()
    svc = ClusterSyncService(mock_repo, mock_sagemaker, ttl_seconds=300)
...
Run: `cd backend && pytest tests/unit/modules/monitoring/test_svc_cluster_sync.py -v`
Expected: FAIL（模块不存在）
...
- [ ] **Step 4: 实现读穿透服务**（含 TTL 判断 + asyncio.Lock 单飞 + 字段映射）

```python
# cluster_sync_service.py — 核心逻辑
import asyncio
...
class ClusterSyncService:
    ...
    async def get_clusters(self) -> list[HyperPodCluster]:
        ...
        async with self._lock:
            ...
            if existing and self._is_fresh(existing):
                return existing
            return await self._sync_from_sagemaker()
    ...
    async def _sync_from_sagemaker(self) -> list[HyperPodCluster]:
        raw = await self._sagemaker.describe_cluster(self._cluster_name)
        entity = self._map_to_entity(raw)  # 字段映射，见下
...
- [ ] **Step 6: 确认 repo 有 list_all/upsert 方法** —— 检查 `IHyperPodClusterRepository`，缺则补接口+实现+测试。
...
git add backend/src/modules/monitoring/application/services/cluster_sync_service.py \
        ...
        backend/src/modules/monitoring/domain/repositories/ \
        backend/src/modules/monitoring/infrastructure/repositories/ \
        backend/tests/unit/modules/monitoring/test_svc_cluster_sync.py
...
- [ ] **Step 1: 写失败集成测试** —— 验证端点存在、返回正确 schema、故障降级 200

```python
@pytest.mark.asyncio
async def test_get_clusters_returns_list(async_client, auth_headers):
    resp = await async_client.get("/api/v1/clusters", headers=auth_headers)
    assert resp.status_code == 200
    body = resp.json()
    assert "items" in body and "total" in body

@pytest.mark.asyncio
async def test_get_utilization_returns_list(async_client, auth_headers):
    resp = await async_client.get("/api/v1/monitoring/utilization", headers=auth_headers)
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_get_alerts_returns_empty_paginated(async_client, auth_headers):
    resp = await async_client.get("/api/v1/monitoring/alerts", headers=auth_headers)
    assert resp.status_code == 200
    body = resp.json()
    assert body["items"] == [] and body["total"] == 0

@pytest.mark.asyncio
async def test_utilization_degrades_to_200_on_amp_failure(async_client, auth_headers, monkeypatch):
    """AMP 故障时端点返回 200 + 空数据，不返回 5xx。"""
    # mock PrometheusService.get_resource_utilization 抛异常
    ...
    resp = await async_client.get("/api/v1/monitoring/utilization", headers=auth_headers)
    assert resp.status_code == 200
    assert resp.json() == []
