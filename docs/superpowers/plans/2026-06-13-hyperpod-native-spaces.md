...
> 这是改动既有工作代码的核心任务。保持现有 65+ 测试绿。策略字典 `dict[SpaceBackend, ISpaceBackendClient]` 注入；配额校验仅 hyperpod。
...
在 spaces 集成测试中加入（若无则创建 `test_repo_space.py`）：
...
- Test: `backend/tests/integration/spaces/test_api_space.py`（扩展）
...
> 集成测试需 mock 掉真实 K8s 调用——在 dependency override 注入 mock backend。参照现有 `test_api_space.py` 如何 override `get_space_service`。
...
**Files:**
- Modify: `specs/001-ai-training-platform/contracts/` 中 spaces 相关 OpenAPI
...
- [ ] **Step 2: 更新 OpenAPI 契约**

在 spaces OpenAPI 契约文件中，create 请求加 `backend`，响应 schema 加 `backend` + HyperPod 字段。
