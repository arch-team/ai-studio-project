...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE resource_limit_configs MODIFY COLUMN role ENUM('ADMIN','PROJECT_MANAGER','ENGINEER','VIEWER') NOT NULL")
    ...
    op.execute("ALTER TABLE audit_logs MODIFY COLUMN operation_type ENUM('CREATE','UPDATE','DELETE','LOGIN','LOGOUT','PAUSE','RESUME','CANCEL') NOT NULL")
    ...
    op.execute("ALTER TABLE audit_logs MODIFY COLUMN resource_type ENUM('TRAINING_JOB','DATASET','MODEL','USER','QUOTA','SPACE') NOT NULL")
