"""Datasets API Contract 一致性验证测试。

验证 FastAPI 生成的 OpenAPI schema 与 contracts/datasets-api.yaml 的一致性。
...
# API 前缀: router.py 中 datasets 挂载在 /api/v1/datasets
API_PREFIX = "/api/v1"


@pytest.fixture
def datasets_contract() -> dict[str, Any]:
    """加载 datasets-api.yaml contract。"""
    return load_contract_yaml("datasets-api.yaml")


class TestDatasetsContractPaths:
    ...
    async def test_openapi_schema_contains_datasets_detail_path(self, openapi_paths: dict[str, Any]) -> None:
        ...
        matching = [
            ...
            if p.startswith(f"{API_PREFIX}/datasets/{{")
            and p.endswith("}")
            and "versions" not in p
            and "fsx" not in p
            and "upload" not in p
            and "health" not in p
        ]
...
class TestDatasetsContractMethods:
    ...
    async def test_datasets_detail_supports_get(self, openapi_paths: dict[str, Any]) -> None:
        ...
        assert detail_path is not None, "datasets 详情路径不存在"
    ...
    async def test_datasets_detail_supports_delete(self, openapi_paths: dict[str, Any]) -> None:
        ...
        assert detail_path is not None, "datasets 详情路径不存在"
        methods = set(openapi_paths[detail_path].keys())
        assert "delete" in methods, "DELETE /datasets/{id} 方法缺失"


class TestDatasetsContractConsistency:
    ...
    async def test_all_contract_paths_exist_in_openapi(
        ...
        datasets_contract: dict[str, Any],
    ) -> None:
        """验证 contract 中定义的所有路径在 OpenAPI schema 中均有对应。"""
        contract_paths = extract_contract_paths(datasets_contract)
