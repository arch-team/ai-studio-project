...
class TestSecurityExceptionHttpStatus:
    ...
    def test_security_error_is_problem_alias(self):
        """SecurityError 现在是 Problem 的别名。"""
        assert SecurityError is Problem
        assert SecurityError.http_status == status.HTTP_400_BAD_REQUEST
...
class TestDomainExceptionGetDetails:
    ...
    def test_resource_quota_exceeded_details(self):
        exc = ResourceQuotaExceededError(resource_type="gpu", limit=10, requested=15)
        details = exc.get_details()
        assert details == {"resource_type": "gpu", "limit": 10, "requested": 15}

    def test_base_problem_details_none(self):
        """Problem 基类无字段时返回 None。"""
        exc = Problem()
        details = exc.get_details()
...
class TestProblemExceptionHandler:
    """Test problem_exception_handler function."""
    ...
    def mock_request_with_trace_id(self):
        ...
        scope = {"type": "http", "method": "GET", "path": "/test"}
        request = Request(scope)
        request.state.trace_id = "test-trace-789"
        return request

    @pytest.mark.asyncio
    async def test_problem_exception_response(self, mock_request):
        """Test problem_exception_handler with a domain exception."""
        exc = EntityNotFoundError(entity_type="Model", entity_id="xyz")
        response = await problem_exception_handler(mock_request, exc)

        assert response.status_code == status.HTTP_404_NOT_FOUND
        content = json.loads(response.body)

        assert content["error"]["code"] == "ENTITY_NOT_FOUND"
        assert "Model" in content["error"]["message"]
        assert content["error"]["details"] == {"entity_type": "Model", "entity_id": "xyz"}

    @pytest.mark.asyncio
    async def test_problem_exception_includes_trace_id(self, mock_request_with_trace_id):
        """Test that problem_exception_handler includes trace_id."""
        exc = EntityNotFoundError(entity_type="Model", entity_id="xyz")
        response = await problem_exception_handler(mock_request_with_trace_id, exc)

        content = json.loads(response.body)
        assert content["error"]["trace_id"] == "test-trace-789"

    @pytest.mark.asyncio
    async def test_problem_handler_with_security_exception(self, mock_request):
        """Test that security exceptions work with problem_exception_handler."""
