...
class CheckpointModel(Base, TimestampMixin):
    ...
    checkpoint_type: Mapped[CheckpointType] = mapped_column(
        Enum(CheckpointType), nullable=False, default=CheckpointType.EPOCH, comment="检查点类型"
    )
    trigger_type: Mapped[CheckpointTriggerType] = mapped_column(
        Enum(CheckpointTriggerType), nullable=False, default=CheckpointTriggerType.SCHEDULED, comment="触发类型"
    )
    ...
    storage_tier: Mapped[StorageTier] = mapped_column(
        Enum(StorageTier), nullable=False, default=StorageTier.FSX, index=True, comment="存储层级"
    )
    status: Mapped[CheckpointStatus] = mapped_column(
        Enum(CheckpointStatus), nullable=False, default=CheckpointStatus.AVAILABLE, index=True, comment="检查点状态"
    )
    ...
    training_job: Mapped["TrainingJobModel"] = relationship(
        "TrainingJobModel", back_populates="checkpoints", foreign_keys=[training_job_id]
    )
