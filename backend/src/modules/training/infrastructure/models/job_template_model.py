...
class JobTemplateModel(Base, TimestampMixin):
    ...
    # Visibility
    visibility: Mapped[TemplateVisibility] = mapped_column(Enum(TemplateVisibility), nullable=False, default=TemplateVisibility.PRIVATE, index=True, comment="可见性范围")
    ...
    last_used_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=False), nullable=True, comment="最后使用时间")

    # Soft delete
    deleted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=False), nullable=True, index=True, comment="软删除时间")
