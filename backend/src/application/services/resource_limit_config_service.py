...
class ResourceLimitConfigService:
    ...
    async def create_config(self, data: dict) -> ResourceLimitConfig:
        ...
        Args:
            data: Config creation data with role, project_id, limits, etc.

        Returns:
            Created ResourceLimitConfig entity

        Raises:
            DuplicateEntityError: If config with same role+project exists
        """
    ...
    async def get_config(self, config_id: int) -> ResourceLimitConfig:
        ...
        if config is None:
            raise EntityNotFoundError("ResourceLimitConfig", str(config_id))
        return config
    ...
    async def update_config(
        ...
        Raises:
            ...
            DuplicateEntityError: If new role+project combination already exists
        """
        config = await self._repository.get_by_id(config_id)
        if config is None:
            raise EntityNotFoundError("ResourceLimitConfig", str(config_id))
        ...
        if new_role != config.role or new_project_id != config.project_id:
            ...
            if existing is not None and existing.id != config_id:
                ...
                raise DuplicateEntityError(
                    "ResourceLimitConfig",
                    f"role={new_role.value}, scope={scope}",
                )
